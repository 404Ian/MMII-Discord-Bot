...
export async function getDefinition(word) {
    ...
    if (cached)
        return cached;
    if (noDefCache.has(lower))
        return null;
    const dictData = await fetchJSON(`https://api.dictionaryapi.dev/api/v2/entries/en/${lower}`);
    if (dictData?.length) {
        const phonetics = dictData[0].phonetics ?? [];
        const phonetic = phonetics.find((p) => p.audio) ?? phonetics[0] ?? null;
        const result = {
            word: lower,
            phonetics: phonetic
            ...
            meanings: dictData[0].meanings.map((m) => ({
    ...
    const freeDictData = await fetchJSON(`https://freedictionaryapi.com/api/v1/entries/en/${lower}`);
        ...
        for (const entry of entries) {
            chosenPron =
                entry.pronunciations?.find((p) => p.type === "ipa" &&
                    p.tags?.some((t) => ["US", "General Australian", "Canada"].includes(t))) ?? entry.pronunciations?.[0] ?? null;
            if (chosenPron)
                break;
        }
        const result = {
            ...
            meanings: entries.map((entry) => ({
                ...
                definitions: entry.senses.map((s) => s.definition),
            })),
        };
        cache.set(lower, result);
        console.log(`${lower}: ${(performance.now() - start).toFixed(2)}ms`);
