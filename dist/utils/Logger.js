 ...
 */
import { ContainerBuilder, WebhookClient, MessageFlags } from "discord.js";
...
export function sanitizeStack(stack) {
    ...
    const cleaned = lines
        .slice(1)
        .map((line) => {
        if (/node:internal|native/.test(line))
            return "";
        if (/Logger\.(ts|js)/.test(line))
            return "";
        return line
            .replace(/^.*file:[/\\]+/, "")
            .replace(/\\/g, "/")
...
export function stackify(stack) {
    if (!stack)
        return "";
...
function log(level, message, time) {
    ...
    const base = space + formatLevel(level) + spacing +
...
// -- Container Presets
function container(level, message, submessage, sep = false) {
    const base = level === "WARN"
        ? `**⚠️ ${message}**`
        : level === "ERROR"
            ? `**‼️ ${message}**`
            : level === "DEBUG"
                ...
                : level === "INFO"
                    ? `${message}`
                    : message;
    const builder = new ContainerBuilder().addTextDisplayComponents(t => t.setContent(base));
    if (submessage) {
        if (sep)
            builder.addSeparatorComponents(sep => sep);
...
export async function channel_log(client, interaction, level, message, submessage) {
    ...
    const container = new ContainerBuilder()
        .addTextDisplayComponents(t => t.setContent(`### ${title}`), t => t.setContent(`-# Happened: <t:${discordTimestamp()}:R>`), t => {
        if ('commandName' in interaction && 'commandId' in interaction) {
            return t.setContent(`-# While using command: </${interaction.commandName}:${interaction.commandId}>`);
        }
        else {
        ...
        .addTextDisplayComponents(t => t.setContent(`**${message}**`), t => t.setContent(`-# ${submessage ? `${submessage}` : 'None'}`))
        .addSeparatorComponents(sep => sep)
        .addTextDisplayComponents(t => t.setContent(`-# Guild: ${interaction.guild ? interaction.guild.name : '`Unknown Guild Name`'} (${interaction.guildId})`), t => t.setContent(`-# Channel ID: ${interaction.channelId}`), t => t.setContent(`-# User: ${interaction.user.username} (${interaction.user.id})`));
    if (color)
        container.setAccentColor(parseInt(color, 16));
    if (config.LogsWebhook && config.LogsWebhook !== "") {
        try {
            const webhookClient = new WebhookClient({ url: config.LogsWebhook });
            try {
    ...
    if (!channel)
        return error("LogsChannel not found in guild cache.");
    try {
        await channel.send({
            components: [container],
            flags: MessageFlags.IsComponentsV2
...
export function warn(message, time = '') {
    log("WARN", message, time);
    write_log('WARN', message + time);
}
export function error(message, time = '') {
    const sanitized = sanitizeStack(time);
    log("ERROR", message, sanitized);
    write_log('ERROR', message + sanitized);
}
export function debug(message, time = '') {
    log("DEBUG", message, time);
    write_log('DEBUG', message + time);
}
export function info_container(message, submessage = '', sep = false) {
    return container('INFO', message, submessage, sep);
}
export function warn_container(message, submessage = '', sep = false) {
    return container('WARN', message, submessage, sep);
}
export function debug_container(message, submessage = '', sep = false) {
    return container('DEBUG', message, submessage, sep);
}
export function error_container(message, submessage = '', sep = false) {
    return container('ERROR', message, submessage, sep);
}
export async function info_log(client, interaction, message, submessage) {
    return await channel_log(client, interaction, 'INFO', message, submessage);
}
export async function warn_log(client, interaction, message, submessage) {
    return await channel_log(client, interaction, 'WARN', message, submessage);
}
export async function debug_log(client, interaction, message, submessage) {
    return await channel_log(client, interaction, 'DEBUG', message, submessage);
}
export async function error_log(client, interaction, message, submessage) {
