import { Events, ContainerBuilder, MessageFlags, ButtonStyle, ButtonBuilder, ButtonInteraction } from "discord.js";
import logger from "../utils/Logger.js";
import PendingCache from "../utils/cache/PendingCache.js";
async function BuildContainer(interaction, type, page, additions, removals) {
    let nextType;
    let buttonLabel;
    ...
    const container = new ContainerBuilder()
        .addTextDisplayComponents(t => t.setContent(`### Pending Words (${type === "all" ? "Additions & Removals" : type === "addition" ? "Additions" : "Removals"})`))
        .addSeparatorComponents(sep => sep)
        .addTextDisplayComponents(t => t.setContent(!content.startsWith(">") ? `\`\`\`diff\n${content}\`\`\`` : content))
        .addSeparatorComponents(sep => sep)
        .addTextDisplayComponents(t => t.setContent(`-# Page ${pageData.page} of ${pageData.totalPages}`));
    return container.addActionRowComponents(row => row.addComponents(new ButtonBuilder()
        .setCustomId(`cmd-page|${type}|first|${userId}`)
        .setLabel("<<")
        .setStyle(ButtonStyle.Secondary)
        .setDisabled(pageData.page === 1), new ButtonBuilder()
        ...
        .setLabel(">")
        .setStyle(ButtonStyle.Secondary)
        .setDisabled(pageData.page === pageData.totalPages), new ButtonBuilder()
        .setCustomId(`cmd-page|${type}|last|${userId}`)
        .setLabel(">>")
        .setStyle(ButtonStyle.Secondary)
        .setDisabled(pageData.page === pageData.totalPages)));
}
async function paginate(words, page) {
...
export async function handlePageInteraction(interaction, type, page) {
...
export async function handleTypeInteraction(interaction, type, page) {
...
export default {
    ...
    async execute(message, client) {
        if (message.author.bot)
            return;
        if (message.channel.id !== "1478448857842188368")
            return;
        ...
        if (cmd === ".view") {
            ...
            const pageArg = args[1] === "first"
                ? "first"
                : args[1] === "last"
                    ? "last"
                    : parseInt(args[1]) || 1;
            const type = typeArg === "add"
                ? "addition"
                : typeArg === "remove"
                    ? "removal"
                    : "all";
