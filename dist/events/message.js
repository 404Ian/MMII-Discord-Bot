// :: /events/onMessage.ts
import { Events, WebhookClient, MessageFlags, ContainerBuilder, ButtonBuilder, ButtonStyle } from 'discord.js';
...
export default {
    ...
    async execute(message, client) {
        if (message.author.bot)
            return;
        if (!["1478425188117905528", "1478425167519678504", "1478448857842188368"].includes(message.channel.id))
            return;
        if (!message.channel.isTextBased() || !('name' in message.channel))
            return;
        if (!/^[a-z]+(?:\s*(?:-|:)\s*[a-z :#-]+|\n-#\s*[a-z :#-]+)$/i.test(message.content))
            return;
        ...
        if (separatorIndex === -1)
            return;
        if (type === "unk")
            return; // Unknown channel type, ignore
        ...
        const existing = pendingCache.getByType("addition")?.[word] ||
            pendingCache.getByType("removal")?.[word];
        if (existing) {
            await message.react("❌");
            const pendingTypeFound = pendingCache.getByType("addition")?.[word] ? "addition" : "removal";
            const reply = await message.reply({
                components: [logger.info_container(`Word is already pending for ${pendingTypeFound}.`)],
                flags: MessageFlags.IsComponentsV2
            });
            setTimeout(() => {
                reply.delete().catch(() => { });
        ...
        try {
            const webhookClient = new WebhookClient({ url: type === "add" ? config.AddWebhook : config.RemoveWebhook });
            try {
        ...
        // Fallback
        const channel = client.channels.cache.get(config.WordsChannel);
        if (!channel)
            return logger.error(`${type} channel not found in cache.`);
        try {
            await message.react("⌛");
            await channel.send({
                components: [container],
