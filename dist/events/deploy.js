...
export default async function deploy(client) {
    ...
    try {
        ...
        for (const guildId of config.Guild_ID) {
            const guildDeployed = await rest.put(Routes.applicationGuildCommands(config.Client_ID, guildId), { body: cmds });
        ...
        logger.info(`Reloaded Events: ${interactions}x Interactions, ${events.join(pc.gray(', '))}`);
        // logger.info(
        //     'Deployed Commands.',
