...
export async function reloadBot() { // reload n reimport
    if (client) {
        client.destroy();
        client = null;
    }
...
async function buildAndReload() { // build n reload
...
function debounce(fn: (...args: any[]) => void, delay: number) { // watcher
...
function startWatcher() {
    ...
    const debounceReload = debounce((_event: string, filePath: string) => {
