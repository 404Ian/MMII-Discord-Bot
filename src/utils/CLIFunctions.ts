...
// ------ Utility Funcs ------ //
function MergerReformat(source: string): string {
    try {
...
function MergerStreamHTTP(url: string, onLine: (line: string) => void) {
...
function MergerStreamFile(file: string, onLine: (line: string) => void) {
...
export async function Merge(overrideMain?: boolean) {
    const allWords = new Set<string>();
    const fileStats: { url: string; count: number; newWords: number }[] = [];
    ...
    console.log(
        pc.yellow(pc.bold(`Total unique words: ${finalWords.length.toLocaleString()}`)), pc.italic(pc.dim(`| written to ${fileToOverwrite}`))
    );
}
